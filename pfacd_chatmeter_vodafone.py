...
st.write("""Esta página apresenta um conjunto de gráficos que fornecem insights valiosos sobre os dados recolhidos,
         referente à <b>Vodafone</b>.""", unsafe_allow_html=True)
st.divider()

# ================================================================================================
# Definir Paletas de Cores da Vodafone
palette_Vodafone = [
    '#730000',  # Vermelho Escuro
    '#e60000',  # Vermelho
    '#f27f80',  # Vermelho Claro
]
...
Facebook_Posts_Analysis = Facebook_Posts_Analysis[Facebook_Posts_Analysis['page'] == 'Vodafone']
...
def apply_filters(filters=None):
    ...
    # Filtro por Posts que incluem 'Vodafone' - Post e PCU
    if filters['post_vodafone_filter']:
        filtered_Post_df = filtered_Post_df[filtered_Post_df['post_text_Vodafone'] == 1]
        filtered_PCU_df = filtered_PCU_df[filtered_PCU_df['post_text_Vodafone'] == 1]

    # Filtro por Comentários que incluem 'Vodafone' - PCU
    if filters['comment_vodafone_filter']:
        filtered_PCU_df = filtered_PCU_df[filtered_PCU_df['comment_text_Vodafone'] == 1]
...
with col3_F:
    # Filtro por Posts que incluem 'Vodafone'
    post_vodafone_filter = st.checkbox("Apenas Posts que Incluem **Vodafone**", value=False)

    # Filtro por Comentários que incluem 'Vodafone'
    comment_vodafone_filter = st.checkbox("Apenas Comentários que Incluem **Vodafone**", value=False)
...
if st.button("Filtrar", on_click=click_button, key='filtros_button'):
    Facebook_PCU_Analysis, Facebook_Posts_Analysis = apply_filters({
        ...
        'end_date': end_date,
        'post_vodafone_filter': post_vodafone_filter,
        'comment_vodafone_filter': comment_vodafone_filter,
        'n_reacts': n_reacts,
...
pfacd_functions.create_card(col1, "fas fa-newspaper", (115, 0, 0), (255, 255, 255),
                            "Total de Posts", Facebook_Posts_Analysis['post_id'].nunique())

# Cartão 2 | Total de Comentários
pfacd_functions.create_card(col2, "fas fa-comments", (230, 0, 1), (255, 255, 255),
                            "Total de Comentários", Facebook_PCU_Analysis['comment_id'].nunique())

# Cartão 3 | Total de Utilizadores Únicos
pfacd_functions.create_card(col3, "fas fa-users", (242, 127, 128), (255, 255, 255),
...
try:
    ...
    with tab1:
        ...
        fig_plot_line.update_traces(line_color='#730000', line_width=1)
        ...
        with col2_P:
            ...
            posts_per_weekday_df = pd.DataFrame({
                'n': posts_per_weekday['Vodafone'],
                '%': posts_per_weekday_percent['Vodafone']
            })
        ...
        with col1_P:
            ...
            pfacd_functions.create_card(col1_T, "fas fa-newspaper", (115, 0, 0), (255, 255, 255),
                                    "Média Diária de Posts", int(Facebook_PCU_Analysis['post_id'].nunique() / Facebook_PCU_Analysis['post_date'].nunique()))

            # Cartão 1.2 | Média de Posts Mensais
            pfacd_functions.create_card(col2_T, "fas fa-newspaper", (230, 0, 1), (255, 255, 255),
                                    "Média Mensal de Posts", int(Facebook_PCU_Analysis['post_id'].nunique() / Facebook_PCU_Analysis['post_date'].dt.month.nunique()))

            # Cartão 1.3 | Média de Posts Anuais
            pfacd_functions.create_card(col3_T, "fas fa-newspaper", (242, 127, 128), (255, 255, 255),
        ...
        fig_plot_bar = px.bar(posts_per_CR_label, x='CR Label', y='Total de Posts',
                              ...
                              color='CR Label',
                              color_discrete_map={'Carece de Resposta': palette_Vodafone[0], 'Não Carece de Resposta': palette_Vodafone[1]})
        ...
        posts_per_SRC_plot = px.area(
            ...
            color_discrete_sequence=palette_Vodafone
        )
    ...
    with tab2:
        ...
        fig_plot_line.update_traces(line_color='#e60000', line_width=1)
        fig_plot_line.update_layout(
            xaxis_title='Data',
        ...
        with col1_C:
            ...
            pfacd_functions.create_card(col1_TC, "fas fa-comments", (115, 0, 0), (255, 255, 255),
            ...
            pfacd_functions.create_card(col2_TC, "fas fa-comments", (230, 0, 1), (255, 255, 255),
            ...
            pfacd_functions.create_card(col3_TC, "fas fa-comments", (242, 127, 128), (255, 255, 255),
        ...
        with col2_C:
            ...
            comments_per_weekday_df = pd.DataFrame({
                'n': comments_per_weekday['Vodafone'],
                '%': comments_per_weekday_percent['Vodafone']
            })
            st.markdown("<br><br>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        pfacd_functions.create_card(col2_U11, "fas fa-users", (115, 0, 0), (255, 255, 255),
        ...
        pfacd_functions.create_card(col2_U11, "fas fa-users", (230, 0, 1), (255, 255, 255),
        ...
        pfacd_functions.create_card(col2_U11, "fas fa-users", (242, 127, 128), (255, 255, 255),
        ...
        df_treemap = df_treemap.groupby(['user_distrito', 'user_concelho', 'user_freguesia'])['user_link'].size().reset_index(name='count')
        ...
        col2_U11.plotly_chart(fig_gender_weekday, use_container_width=True)
